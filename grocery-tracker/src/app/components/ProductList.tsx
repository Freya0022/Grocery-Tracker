...
const ProductList: React.FC<ProductListProps> = ({ productList }) => {
  ...
  const sortedProductList = useMemo(() => {
    ...
    if (sortBy === "bestBeforeAsc") {
      ...
      return (
        new Date(b.bestBefore).getTime() - new Date(a.bestBefore).getTime()
      );
  ...
  const totalCount = sortedProductList.length;
  ...
  const handlePaginate = (page: number) => {
    setCurrentPage(page);
    setDisplayList(sortedProductList.slice(startIndex, endIndex));
  };

  useEffect(() => {
    setDisplayList(sortedProductList.slice(startIndex, endIndex));
  }, [sortedProductList,currentPage]);
        ...
        <div className="flex items-center justify-between text-gray-500 mb-4">
          <div>
            ...
            <select
              ...
              className="border ml-2 rounded py-1 text-black"
            >
          ...
          <div>
            <p>
              <span className="text-black font-[500] pr-1">
